...
def setup_function():
    ...
    for table in ("user_profile", "policy", "team_order", "rating", "par_level"):
        app.conn.execute(f"delete from {table}")
    app.conn.commit()


def test_solve_tool_excludes_the_restaurant_a_jain_teammate_cannot_eat_at():
    db.upsert_profile(app.conn, "U1", "jain", [], None)
    db.upsert_profile(app.conn, "U2", "nonveg", [], None)
...
def test_solve_tool_picks_and_advances_the_state_machine():
    db.upsert_profile(app.conn, "U1", "veg", [], None)
    state = lunch.open_lunch(CHANNEL, "1.1")
    lunch.join(state, "U1")
...
def test_solve_tool_respects_a_channel_budget_cap():
    db.upsert_profile(app.conn, "U1", "nonveg", [], None)
    db.upsert_policy(app.conn, CHANNEL, 150, [])
    state = lunch.open_lunch(CHANNEL, "1.1")
    lunch.join(state, "U1")
...
def test_pantry_tool_diffs_against_par_and_stores_the_draft():
    db.set_par_level(app.conn, "p1", "Milk 1L", 6)
    db.set_par_level(app.conn, "p2", "Coffee 200g", 2)
